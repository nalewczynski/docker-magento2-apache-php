...
if [ ! -d /var/www/html/magento2 ]; then
  ...
  php $magento_path/bin/magento setup:install \
    --db-host ${COMPOSE_PROJECT_NAME}_db_1 --db-name magento2 --db-user magento2 --db-password magento2 --admin-user admin --timezone $TIMEZONE --currency $CURRENCY --use-rewrites 1 --cleanup-database \
